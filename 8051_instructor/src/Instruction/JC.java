...
public class JC extends OperatorSuper{
    public JC(ArrayList<String> op, int StepOrRun) throws WrongException{
        super(op, 1, "JC", StepOrRun);
    }

    @Override
    public void judgeOperandIsValid(int StepOrRun) throws WrongException {
        if(StepOrRun == ASSEMBLY){
            CodeMemory.updateCodeMemoryBeginPos(31);
            return;
        }
        ...
        if(Label.checkIsLabelExist(operand.get(0))){
            ...
            if(StepOrRun == STEP)
                Instruction.updateEncoding(31, CarrySystem.BinarytoHex("01000000") + offset);
            else{
                if(register.getRegister("C").equals("1"))
