...
public class JNB extends OperatorSuper{
    public JNB(ArrayList<String> op, int StepOrRun) throws WrongException{
        super(op, 2, "JNB", StepOrRun);
    }

    @Override
    public void judgeOperandIsValid(int StepOrRun) throws WrongException {
        if(StepOrRun == ASSEMBLY){
            CodeMemory.updateCodeMemoryBeginPos(24);
            return;
        }
        ...
        if(Label.checkIsLabelExist(operand.get(1)) && judgeIsValid.isdirectNumber(operand.get(0))){
            ...
            if(StepOrRun == STEP)
                Instruction.updateEncoding(24, CarrySystem.BinarytoHex("00110000" + CarrySystem.directNumbertoBinary(operand.get(0))) + offset);
            else{
                ...
                if(CarrySystem.HextoBinary(DataMemory.getDataMemory(Integer.toHexString(Clocation)), 8).substring(Cbit, Cbit+1).equals("0"))
